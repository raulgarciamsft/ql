...
class InitializationFunction extends Function {
  ...
  InitializationFunction() {
    ...
    (
      exists(Assignment e | e.getEnclosingFunction() = this |
        e.getLValue().(PointerDereferenceExpr).getOperand() = this.getParameter(i).getAnAccess()
        or
        // Field wise assignment to the parameter
        e.getLValue() = getAFieldAccess(this.getParameter(i))
      )
      or
      i = this
            .(MemberFunction)
...
/**
 * Holds if the call should not be considered as initializing a parameter.
 */
predicate isWhitelistedCall(Call c) {
  c.getTarget().getName().matches("FastGetProfile%") and
  c.getArgument(5).getValue() = "0"
  or
  /*
   * RtlUnicodeStringToInteger only leaves the first argument uninitialized if the last argument is
   * null. If the last argument is computed using the "address of" operator, it cannot be null.
   */

  c.getTarget().getName().matches("RtlUnicodeStringToInteger") and
  c.getArgument(2) instanceof AddressOfExpr
  or
  /*
   * RtlInitUnicodeStringEx only fails if the length of the string passed as the initializer is
   * greater than the max length. If the argument is a string literal, this is assumed not to be
   * true.
   */

  c.getTarget().getName().matches("RtlInitUnicodeStringEx") and
  c.getArgument(1) instanceof StringLiteral
  or
  /*
   * ConvertToBlendDesc always initializes the second argument if the first argument is not null.
   */

  c.getTarget().hasName("ConvertToBlendDesc") and
  c.getArgument(0) instanceof AddressOfExpr
}
