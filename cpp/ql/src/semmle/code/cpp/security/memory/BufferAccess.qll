...
class Alloc extends Function {
  Alloc() {
    this.getName().matches("UserAlloc%") or
    this.hasName("ExAllocatePoolWithTag") or
    this.hasName("HeapAlloc") or
    this.hasName("GlobalAlloc") or
    this.hasName("malloc") or
    this.hasName("PALLOCMEM2") or
    this.hasName("AllocFreeTmpBuffer") or
    this.getName().matches("Win32Alloc%")
  }
}
