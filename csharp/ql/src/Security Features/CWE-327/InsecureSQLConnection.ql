...
class UnencryptedSQLConnection_DataflowConfiguration extends TaintTracking::Configuration {
  UnencryptedSQLConnection_DataflowConfiguration() {
    this = "UnencryptedSQLConnection_DataflowConfiguration"
  }
...
from
  UnencryptedSQLConnection_DataflowConfiguration c, DataFlow::Node source, DataFlow::Node sink,
  ObjectCreation oc
where
  source != sink and
  c.hasFlow(source, sink) and
  oc.getRuntimeArgument(0) = sink.asExpr()
